...
public class moderator extends Controller {
	...
	public static void postarticle(String title,String categoryName,String tagName,String departmentName,String articleContent,boolean enabled,String author,String articleType,Long id) {
		...
		Accounts c = Accounts.find("byName",author).first();
		index(c.id);
...
public static void search(String keyword,Long a) {
	...
	public static void notifications(Long a) {
			List com = AllArticles.find("byId",a).fetch();
			List comm =  Comment.find("AccountId = ? AND isEnabled = false",a).fetch();
			
			//Comment com = Comment.find("by",).first(); 
			
				Accounts c =Accounts.find("byId",a).first();
				System.out.println(c.id);
				render(c,comm,com);
	}
	public static void publishcomment(Long id, Long a) {
		Comment art = Comment.findById(id);
		art.isEnabled = true;
		art.save();
		Accounts c =Accounts.find("byId",a).first();
		notifications(c.id);
	...
	public static void deletecomment(Long b,Long a) {
		System.out.println(b);
		//Comment com = Comment.find("byId",b);
		//com.delete();
		Accounts c =Accounts.find("byId",a).first();
		notifications(c.id);
	}
